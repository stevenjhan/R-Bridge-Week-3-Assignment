...
## Hidden R Code
## I had a question on below for loop which I was unable to execute properly
## In the below, only the last iteration in both for loops evaluates--could you help me understand why?

func_vector <- c("calc_combination", "choose")
set_total_nums <- c(5, 10, 2, 15)
set_subset_nums <- c(3, 0, 4, 7)
table_results <- data.frame(Set_Total=as.numeric(), Subset=as.numeric(), calc_combination=as.numeric(), choose=as.numeric())

for (i in length(set_total_nums)) {
    table_results[i, "Set_Total"] <- set_total_nums[i]
    table_results[i, "Subset"] <- set_subset_nums[i]
    
    for (n in length(func_vector)) {
